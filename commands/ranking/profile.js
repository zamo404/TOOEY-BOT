...
module.exports = {
  ...
  //  let member = await message.mentions.members.first() || message.author
   ...
   //let member = (await message.mentions.members.first()) || message.author;
    ...
    const server_rank = await profile.find({ 'xp.id': message.guild.id })
      .then(docs => Promise.resolve(docs.sort((A,B) => B.xp.find(x => x.id === message.guild.id).xp - A.xp.find(x => x.id === message.guild.id).xp)))
      .then(sorted => sorted.findIndex(x => x.userID === doc.userID) + 1);

   //   const server_data = doc.xp.find(x => x.id === message.guild.id);
