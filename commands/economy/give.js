...
module.exports = {
 ...
 if(!isNaN(money)) return message.reply({content:`You must be a number your credit to give `})
    ...
    if(! money) return message.channel.send({content:`Type credit!`})
  ...
  member.send({content: `🏧 | Transfer Receipt \`You have received $${args[2]} From user ${message.author.username} (ID: ${message.author.id})\``})
    message.channel.send({content:`**${message.author.username}** send credit to **${member.username}** amount: \`$${args[2]}${m}\``})
    }};
