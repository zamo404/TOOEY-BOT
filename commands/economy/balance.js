  ...
  run: async (client, message, args, dev, dev2) => {
    ...
    if (member) {
      ...
      message.channel.send({
        content:`
          <:ano_40:932239945328914433> **${member.username}**, credits balance is __${autho.money}__ ${m}`
      });
    }
    if (!member) {
      let author = await User.findOne({ userID: message.author.id });
      message.reply({
        content: `<:ano_40:932239945328914433> **${message.author.username}**, Your credits balance is:  __\`${author.money}\`__${m}
      `});
    }
  }
};
